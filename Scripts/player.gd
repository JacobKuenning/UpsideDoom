extends CharacterBody3D
var projectile = preload("res://Scenes/player_projectile.tscn")
@onready var audio_players: Node3D = $AudioPlayers
@onready var player_hit: AnimatedSprite3D = $CameraController/PlayerHit
@onready var player_hit_audio: AudioStreamPlayer3D = $PlayerHit
@onready var gm = get_node("/root/GameManager")
@export var sm : Node
@export var flipcd : Timer
@export var guntimer : Timer
@export var as3D : AnimatedSprite3D
@export var camcom : Node3D
var flipped = false
var moving = false
var can_shoot = true
var shooting = false
var audio_index = 0

@export var SPEED = 15
@export var ACCEL = 2
@export var FRIC = 1
@export var FLIP_ACCEL_WIN = 0.1 #how much time after flip the player is given max acceleration
@export var flip_cooldown = 1
@export var mouse_sens = 50
@export var projectile_speed = 40
@export var time_between_shots = 0.2

var look_dir: Vector2

func _ready():
	for state in sm.get_children(): #populating the normal and flipped states variables to be the ones set in the editor
		state.SPEED = SPEED
		state.ACCEL = ACCEL
		state.FRIC = FRIC
		state.mouse_sens = mouse_sens
		state.FLIP_ACCEL_WIN = FLIP_ACCEL_WIN
	flipcd.wait_time = flip_cooldown
	guntimer.wait_time = time_between_shots
	

func _physics_process(delta):
	pass
	
func _input(event: InputEvent): #handle mouse motion and other button inputs
	if event.is_action_pressed("shoot") and can_shoot:
		shooting = true
		fire_weapon()
	
	if event.is_action_released("shoot"):
		shooting = false
		guntimer.start()
		
func fire_weapon():
	print("fire weapon")
	guntimer.start()
	
	var audio_player = audio_players.get_child(audio_index)
	audio_player.pitch_scale = randf_range(.5, 1.5)
	audio_player.play()
	audio_index += 1
	if audio_index == 3:
		audio_index = 0
	
	as3D.play(&"Firing")
	can_shoot = false
	var new_proj = projectile.instantiate()
	gm.projectiles.add_child(new_proj)
	if !GameManager.flipped:
		new_proj.position = Vector3(camcom.global_position.x, camcom.global_position.y - 0.3, camcom.global_position.z)
	else:
		new_proj.position = Vector3(camcom.global_position.x, camcom.global_position.y + 0.3, camcom.global_position.z)

	new_proj.rotation.y = rotation.y
	new_proj.speed = projectile_speed


func _on_area_3d_area_entered(area: Area3D) -> void:
	player_hit_audio.pitch_scale = randf_range(.8, 1.2)
	player_hit_audio.play()
	player_hit.play("default")
	GameManager.game_manager.decrement_health()
	if area.get_parent().name != "ExitArea":
		area.get_parent().queue_free() #destroy bullets that touch you
	pass # Replace with function body.


func _on_gun_timer_timeout() -> void:
	as3D.play(&"Idle")
	can_shoot = true
	if shooting:
		fire_weapon()
	pass # Replace with function body.
